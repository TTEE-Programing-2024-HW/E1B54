...
/*這次花最多時間的是第三題,第一次時我只能打出數字的三角形而且還是反的,但測試很多次後終於成功了
我用的方法是最外層for先控制行數,再用第二個控制前面的空白數量,第三個用來控制印出的字元
雖然研究很久但打出來很有成就感,本來很灰心認為自己沒天分也不像很多同學很快就能搞懂邏輯並打出程式
但現在靠自己慢慢打出來然後清楚每個環節的作用讓我覺得很有趣,希望能讓我對程式的熱情能增加. 
